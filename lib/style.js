...
module.exports = {
  ...
  problems(problems) {
    ...
    // track this internally because webpack stats is inaccurate
    const counts = { problems: 0, errors: 0, warnings: 0 };
    ...
    for (const key of Object.keys(problems)) {
      ...
      for (const type of Object.keys(types)) {
        ...
        for (const item of problem[type]) {
          ...
          rows.push(['', dim(`${item.line}:${item.column}`), probType, lines[0]]);
